...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔐 GOOGLE SOLUTION FORGOT PASSWORD REQUEST: ${email}`);
    ...
    // Google-style solution - use a working email service
    const sendGridApiKey = process.env.SENDGRID_API_KEY;
    const frontendUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.askyacham.com';

    console.log('📧 GOOGLE SOLUTION EMAIL DELIVERY:');
    ...
    // Google-style solution - use a working email service with verified sender
    const emailData = {
      personalizations: [{
        to: [{ email: email }],
        subject: 'Reset your password - Ask Ya Cham',
        custom_args: {
          source: 'forgot-password-google-solution',
          timestamp: Date.now().toString()
        }
      }],
      from: { 
        email: 'info@askyacham.com', // Use your verified sender
    ...
    console.log(`✅ GOOGLE SOLUTION PASSWORD RESET EMAIL SENT: ${email} - Message ID: ${messageId}`);
    ...
    console.error('❌ GOOGLE SOLUTION FORGOT PASSWORD ERROR:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
