...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔐 WORKING NOW 8 FORGOT PASSWORD REQUEST: ${email}`);
    ...
    // Working now 8 approach - use a working email service
    const sendGridApiKey = process.env.SENDGRID_API_KEY;
    const frontendUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.askyacham.com';

    console.log('📧 WORKING NOW 8 EMAIL DELIVERY:');
    ...
    // Working now 8 email with verified sender
    const emailData = {
      personalizations: [{
        to: [{ email: email }],
        subject: 'Reset your password - Ask Ya Cham',
        custom_args: {
          source: 'forgot-password-working-now-8',
    ...
    console.log(`✅ WORKING NOW 8 PASSWORD RESET EMAIL SENT: ${email} - Message ID: ${messageId}`);
    ...
    console.error('❌ WORKING NOW 8 FORGOT PASSWORD ERROR:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
