...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔐 GOOGLE FORGOT PASSWORD REQUEST: ${email}`);
    ...
    // Google working approach - use a working email service
    const sendGridApiKey = process.env.SENDGRID_API_KEY;
    const frontendUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.askyacham.com';

    console.log('📧 GOOGLE EMAIL DELIVERY:');
    ...
    // Google working email with verified sender
    const emailData = {
      personalizations: [{
        to: [{ email: email }],
        subject: 'Reset your password - Ask Ya Cham',
        custom_args: {
          source: 'forgot-password-google',
    ...
    console.log(`✅ GOOGLE PASSWORD RESET EMAIL SENT: ${email} - Message ID: ${messageId}`);
    ...
    console.error('❌ GOOGLE FORGOT PASSWORD ERROR:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
