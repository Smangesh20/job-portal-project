...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔐 ULTIMATE FORGOT PASSWORD REQUEST: ${email}`);
    ...
    // Ultimate working approach - use a working email service
    const sendGridApiKey = process.env.SENDGRID_API_KEY;
    const frontendUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.askyacham.com';

    console.log('📧 ULTIMATE EMAIL DELIVERY:');
    ...
    // Ultimate working email with verified sender
    const emailData = {
      personalizations: [{
        to: [{ email: email }],
        subject: 'Reset your password - Ask Ya Cham',
        custom_args: {
          source: 'forgot-password-ultimate',
    ...
    console.log(`✅ ULTIMATE PASSWORD RESET EMAIL SENT: ${email} - Message ID: ${messageId}`);
    ...
    console.error('❌ ULTIMATE FORGOT PASSWORD ERROR:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
