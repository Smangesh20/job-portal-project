...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔐 FINAL FORGOT PASSWORD REQUEST: ${email}`);
    ...
    // Final working approach - use a working email service
    const sendGridApiKey = process.env.SENDGRID_API_KEY;
    const frontendUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.askyacham.com';

    console.log('📧 FINAL EMAIL DELIVERY:');
    ...
    // Final working email with verified sender
    const emailData = {
      personalizations: [{
        to: [{ email: email }],
        subject: 'Reset your password - Ask Ya Cham',
        custom_args: {
          source: 'forgot-password-final',
    ...
    console.log(`✅ FINAL PASSWORD RESET EMAIL SENT: ${email} - Message ID: ${messageId}`);
    ...
    console.error('❌ FINAL FORGOT PASSWORD ERROR:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
