...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔐 SIMPLE FORGOT PASSWORD REQUEST: ${email}`);
    ...
    // Simple working approach - use a working email service
    const sendGridApiKey = process.env.SENDGRID_API_KEY;
    const frontendUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.askyacham.com';

    console.log('📧 SIMPLE EMAIL DELIVERY:');
    ...
    // Simple working email with verified sender
    const emailData = {
      personalizations: [{
        to: [{ email: email }],
        subject: 'Reset your password - Ask Ya Cham',
        custom_args: {
          source: 'forgot-password-simple',
    ...
    console.log(`✅ SIMPLE PASSWORD RESET EMAIL SENT: ${email} - Message ID: ${messageId}`);
