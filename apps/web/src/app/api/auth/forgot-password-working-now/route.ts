...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔐 WORKING NOW FORGOT PASSWORD REQUEST: ${email}`);
    ...
    // Working now approach - use a working email service
    const sendGridApiKey = process.env.SENDGRID_API_KEY;
    const frontendUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.askyacham.com';

    console.log('📧 WORKING NOW EMAIL DELIVERY:');
    ...
    // Working now email with verified sender
    const emailData = {
      personalizations: [{
        to: [{ email: email }],
        subject: 'Reset your password - Ask Ya Cham',
        custom_args: {
          source: 'forgot-password-working-now',
    ...
    console.log(`✅ WORKING NOW PASSWORD RESET EMAIL SENT: ${email} - Message ID: ${messageId}`);
    ...
    console.error('❌ WORKING NOW FORGOT PASSWORD ERROR:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
