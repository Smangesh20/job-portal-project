...
export async function GET(request: NextRequest) {
  try {
    ...
    // 🚀 HANDLE OAUTH ERRORS
    if (error) {
      console.error('Force Consent OAuth Error:', error)
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=force_consent_auth_failed`)
    }
    ...
    const tokenResponse = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({
        client_id: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || '656381536461-b7alo137q7uk9q6qgar13c882pp4hqva.apps.googleusercontent.com',
        client_secret: process.env.GOOGLE_CLIENT_SECRET || '',
        code: code,
        grant_type: 'authorization_code',
        redirect_uri: `${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/api/auth/google/force-consent/callback`,
      }),
    })
    ...
    // 🚀 CREATE USER SESSION - FORCE CONSENT SUCCESS
    const userSession = {
      id: userData.id,
    ...
    redirectUrl.searchParams.set('action', 'force_consent_signup')
    redirectUrl.searchParams.set('state', state || '')
    redirectUrl.searchParams.set('user_email', userData.email)
    redirectUrl.searchParams.set('user_name', userData.name || '')
    redirectUrl.searchParams.set('auth_method', 'google_force_consent')
    ...
    console.error('🚨 Force consent callback error:', error)
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=force_consent_callback_failed`)
  }
}
