...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=google_auth_failed`)
    }
    ...
    // 🚀 FORCE SIGNUP ACTION
    const action = 'signup'
    console.log('🚀 FORCE CONSENT CALLBACK - Action:', action, 'State:', state, 'Code:', code)
    ...
    console.error('🚨 Google force consent callback error:', error)
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=google_callback_failed`)
  }
}
