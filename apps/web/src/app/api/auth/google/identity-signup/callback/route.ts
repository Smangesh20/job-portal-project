...
export async function GET(request: NextRequest) {
  try {
    ...
    // 🚀 HANDLE OAUTH ERRORS
    if (error) {
      console.error('Identity OAuth Error:', error)
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=identity_auth_failed`)
    }
    ...
    const tokenResponse = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/api/auth/google/identity-signup/callback`,
      }),
    })
    ...
    // 🚀 CREATE USER SESSION - IDENTITY SIGNUP SUCCESS
    const userSession = {
      id: userData.id,
    ...
    console.error('🚨 Identity callback error:', error)
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=identity_callback_failed`)
  }
}