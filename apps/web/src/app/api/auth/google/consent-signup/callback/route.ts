...
export async function GET(request: NextRequest) {
  try {
    ...
    // 🚀 HANDLE OAUTH ERRORS
    if (error) {
      console.error('Consent OAuth Error:', error)
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=consent_auth_failed`)
    }
    ...
    const tokenResponse = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/api/auth/google/consent-signup/callback`,
      }),
    })
    ...
    // 🚀 CREATE USER SESSION - CONSENT SIGNUP SUCCESS
    const userSession = {
      id: userData.id,
    ...
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=consent_callback_failed`)
  }
}
