...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=legacy_auth_failed`)
    }
    ...
    const tokenResponse = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/api/auth/google/legacy-signup/callback`,
      }),
    })
    ...
    // 🚀 CREATE USER SESSION - LEGACY SIGNUP SUCCESS
    const userSession = {
      id: userData.id,
    ...
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=legacy_callback_failed`)
  }
}
