...
export async function GET(request: NextRequest) {
  try {
    ...
    // 🚀 HANDLE OAUTH ERRORS
    if (error) {
      console.error('Ultimate OAuth Error:', error)
      return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=ultimate_auth_failed`)
    }
    ...
    const tokenResponse = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/api/auth/google/ultimate-signup/callback`,
      }),
    })
    ...
    // 🚀 CREATE USER SESSION - ULTIMATE SIGNUP SUCCESS
    const userSession = {
      id: userData.id,
    ...
    console.error('🚨 Ultimate callback error:', error)
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL || 'https://www.askyacham.com'}/signup?error=ultimate_callback_failed`)
  }
}
