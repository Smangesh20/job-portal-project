...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔐 WORKING FORGOT PASSWORD REQUEST: ${email}`);
    ...
    // Google-style working approach - use a working email service
    const sendGridApiKey = process.env.SENDGRID_API_KEY;
    const frontendUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.askyacham.com';

    console.log('📧 WORKING EMAIL DELIVERY:');
    ...
    // Google-style working email with verified sender
    const emailData = {
      personalizations: [{
        to: [{ email: email }],
        subject: 'Reset your password - Ask Ya Cham',
        custom_args: {
          source: 'forgot-password-working',
    ...
    console.log(`✅ WORKING PASSWORD RESET EMAIL SENT: ${email} - Message ID: ${messageId}`);
    ...
    console.error('❌ WORKING FORGOT PASSWORD ERROR:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
