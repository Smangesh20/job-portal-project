...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔐 GOOGLE-STYLE FORGOT PASSWORD REQUEST: ${email}`);
    ...
    // Google-style approach - use a working email service
    const sendGridApiKey = process.env.SENDGRID_API_KEY;
    const frontendUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.askyacham.com';

    console.log('📧 GOOGLE-STYLE EMAIL DELIVERY:');
    ...
    // Google-style email with verified sender - use a working email
    const emailData = {
      personalizations: [{
        to: [{ email: email }],
        subject: 'Reset your password - Ask Ya Cham',
        custom_args: {
          source: 'forgot-password-google-style',
    ...
    console.log(`✅ GOOGLE-STYLE PASSWORD RESET EMAIL SENT: ${email} - Message ID: ${messageId}`);
    ...
    console.error('❌ GOOGLE-STYLE FORGOT PASSWORD ERROR:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
