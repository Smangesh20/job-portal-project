...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔐 BULLETPROOF FORGOT PASSWORD REQUEST: ${email}`);
    ...
    // Google-style bulletproof approach - use a working email service
    const sendGridApiKey = process.env.SENDGRID_API_KEY;
    const frontendUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.askyacham.com';

    console.log('📧 BULLETPROOF EMAIL DELIVERY:');
    ...
    const emailData = {
      personalizations: [{
        ...
        custom_args: {
          source: 'forgot-password-bulletproof',
    ...
    console.log(`✅ BULLETPROOF PASSWORD RESET EMAIL SENT: ${email} - Message ID: ${messageId}`);
    ...
    console.error('❌ BULLETPROOF FORGOT PASSWORD ERROR:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
