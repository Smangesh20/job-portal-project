...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🚀 WORKING EMAIL TEST:')
    ...
    if (!sendGridApiKey) {
      return NextResponse.json({
        ...
        instructions: {
          step1: 'Add SENDGRID_API_KEY to your environment variables',
          step2: 'Add FROM_EMAIL to your environment variables',
    ...
    const msg = {
      ...
      subject: '🚀 WORKING EMAIL TEST - Your SendGrid is Working!',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: white; padding: 40px; border-radius: 0 0 10px 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            <h2 style="color: #333; margin-top: 0;">🎉 WORKING EMAIL TEST SUCCESSFUL!</h2>
            ...
            <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
              ...
              <ul style="color: #666; margin: 0; padding-left: 20px;">
                ...
                <li><strong>API Key Length:</strong> ${sendGridApiKey?.length} characters</li>
      ...
      `,
      text: `🚀 Ask Ya Cham - WORKING Email Test Successful!\n\nCongratulations! Your SendGrid integration is working perfectly.\n\nRecipient: ${email}\nSender: ${fromEmail}\nAPI Key Length: ${sendGridApiKey?.length} characters\nTimestamp: ${new Date().toLocaleString()}\nStatus: ✅ Delivered Successfully\n\nVisit: https://askyacham.com`
    }

    console.log('🚀 Sending working email test...')
    
    // 🚀 SEND EMAIL
    const response = await sgMail.send(msg)
    
    console.log('📧 Working email test sent successfully!')
    console.log('📧 SendGrid response:', response)

    return NextResponse.json({
      success: true,
      message: 'Working email test sent successfully',
      data: {
        email,
        fromEmail,
    ...
    return NextResponse.json({
      success: false,
      error: 'Failed to send working email test',
