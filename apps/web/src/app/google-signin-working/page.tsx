...
export default function GoogleSignInWorking() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      console.log('🚀 GOOGLE SIGN-IN CLICKED - WORKING!')
      ...
      if (data.success && data.data.authUrl) {
        ...
        // Immediate redirect like Google
        setTimeout(() => {
  ...
  const testEmailNow = async () => {
    setIsLoading(true)
    setStatus('🚀 Sending Email NOW...')

    try {
      const response = await fetch('/api/email-working-now', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
