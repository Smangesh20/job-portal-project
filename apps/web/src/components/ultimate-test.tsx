...
import { getUltimateWelcomeMessage, setUltimateUserName, autoDetectAndSetUserName } from '@/utils/ultimate-welcome'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { useAuthUnified } from '@/hooks/useAuthUnified'

export function UltimateTest() {
  ...
  const handleSetName = () => {
    if (testName.trim()) {
      setUltimateUserName(testName.trim())
  ...
  const handleAutoDetect = () => {
    const detectedName = autoDetectAndSetUserName()
  ...
  return (
    <div className="space-y-6">
      <Card className="border-2 border-green-200 bg-green-50/50">
        <CardHeader>
          <CardTitle className="text-green-800">🔧 ULTIMATE DROPDOWN TEST - NO CSS HOVER!</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="p-4 bg-white rounded-lg border">
            ...
            <p className="text-sm text-gray-600 mb-3">
              Current welcome message: <strong>{getUltimateWelcomeMessage(user)}</strong>
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <UltimateDropdown
              label="Job Types (Multiple Selection) - NO HOVER, ONLY CLICK"
              options={jobTypes}
            ...
            <UltimateSingleDropdown
              label="Location (Single Selection) - NO HOVER, ONLY CLICK"
              options={locations}
          ...
          <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
            <h3 className="text-lg font-semibold text-blue-800 mb-2">🔧 ULTIMATE INSTRUCTIONS</h3>
